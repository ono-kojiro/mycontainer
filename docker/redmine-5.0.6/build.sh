...
restore()
{
  docker cp backup/files.tar.bz2  redmine-5.0.6:/tmp/
  docker cp backup/postgres.dump  postgres-16.1:/tmp/
  docker cp backup/template1.dump postgres-16.1:/tmp/
...
EOF
  ...
  docker compose --env-file env exec -T postgres bash -s << EOF
  {
